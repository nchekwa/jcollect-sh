...
system_info_dump () {
cli -c "show system information | display json" > $file_path/system_information.json
cli -c "show system information | display xml" > $file_path/system_information.xml
serialnumber=$(cat $file_path/system_information.xml | grep serial-number | awk -F">" '{print $2}' | awk -F"<" '{print $1}')
hardwaremodel=$(cat $file_path/system_information.xml | grep hardware-model | awk -F">" '{print $2}' | awk -F"<" '{print $1}')
}
...
system_info_dump
echo ">--"
echo "--- Collector start at:  $date_time | $(date '+%s')"
...
# =================================================================================================
: '
# dummy TASK [just in case of testing]
action="DUMMY TASK"
print_status
sleep 1 &
PID=$(echo $!)
...
do
    ...
    then
        ...
        a=$(echo "$j_file_noExt" | sed -e "s/^junos_[a-zA-Z0-9]*_//" -e "s/_/ /")
        action="${a} [EXTERNAL]"
...
print_status
#sleep 45 &
cli -c "request support information | save $file_path/RSI_$hostname_$date_time_fname.txt" > /dev/null 2>&1 &
PID=$(echo $!)
run_background
...
print_status
#sleep 45 &
cd /var/; tar -zcvf $file_path/LOGS_$hostname_$date_time_fname.tgz log/* > $file_path/LOGS_TAR.log 2>&1 &
PID=$(echo $!)
run_background
